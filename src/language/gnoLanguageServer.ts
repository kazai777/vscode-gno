...
export function buildLanguageServerConfig(gnoConfig: vscode.WorkspaceConfiguration): LanguageServerConfig {
	...
	if (!languageServerPath) {
		// Desactiver le LSP si le binaire n'est pas trouver
		cfg.enabled = false;
		return cfg;
	}

	cfg.path = languageServerPath;
	cfg.serverName = 'gnopls';
...
export function getLanguageServerToolPath(): string | undefined {
	return getGnoplsBinaryPath();
    // Obtenir le chemin racine de l'extension
    /*const extension = vscode.extensions.getExtension(extensionId);
    if (!extension) {
        vscode.window.showErrorMessage("Extension introuvable.");
        return undefined;
    }
    const extensionRoot = extension.extensionPath;

    // Construire le chemin vers 'lsp/gnopls' dans le répertoire racine de l'extension
    let gnoplsBinaryPath = path.join(extensionRoot, 'gnopls', 'gnopls');

    // Vérifier si le binaire gnopls existe à cet emplacement
    if (fs.existsSync(gnoplsBinaryPath)) {
        return gnoplsBinaryPath;
    } else {
        vscode.window.showErrorMessage(`Le binaire gnopls n'a pas été trouvé à l'emplacement ${gnoplsBinaryPath}`);
        return undefined;
    }*/
}
