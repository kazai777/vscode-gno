...
export const updateGnoplsBinary: CommandFactory = (ctx, gnoCtx) => {
	return async () => {
		try {
			...
			const gnoFunctionsDir = path.join(extensionPath, 'gnopls', 'gno_functions');

			if (!fs.existsSync(gnoFunctionsDir)) {
				vscode.window.showErrorMessage(`The gno_functions folder was not found in the location ${gnoFunctionsDir}`);
			...
			if (!fs.existsSync(makefilePath)) {
				vscode.window.showErrorMessage(`Makefile not found at location ${makefilePath}`);
			...
			await vscode.window.withProgress(
				...
				async (progress) => {
					...
					await new Promise<void>((resolve, reject) => {
						const gitPullProcess = cp.exec('git pull', { cwd: gnoFunctionsDir }, (error, stdout, stderr) => {
							if (error) {
								vscode.window.showErrorMessage(`Error running git pull: ${stderr || error.message}`);
								reject(error);
								return;
							}
					...
					await new Promise<void>((resolve, reject) => {
						const makeProcess = cp.exec('make build', { cwd: makefileDir }, (error, stdout, stderr) => {
							if (error) {
								vscode.window.showErrorMessage(`Error executing Makefile: ${stderr || error.message}`);
								reject(error);
								return;
							}
							console.log(`[make stdout]: ${stdout}`);
							console.log(`[make stderr]: ${stderr}`);
							vscode.window.showInformationMessage(`Gnopls binary updated successfully!`);
							resolve();
						});
